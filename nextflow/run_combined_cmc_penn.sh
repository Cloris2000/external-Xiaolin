#!/bin/bash
# Launch CMC_PENN combined pipeline with proper settings to avoid NFS issues
...
WORK_DIR="/external/rprshnas01/netdata_kcni/stlab/Xiaolin/nextflow/work_combined_CMC_PENN"

echo "================================================"
echo "Running CMC_PENN Combined Pipeline"
echo "================================================"
echo "Work directory (temporary): $WORK_DIR"
echo "Output directory (permanent): results/CMC_PENN/"
echo "================================================"
echo ""
...
/nethome/kcni/xzhou/.local/bin/nextflow run combined_pipeline.nf \
    -c nextflow.config.combined.cmc_penn \
    -w "$WORK_DIR" \
    -name "CMC_PENN_$$" \
    -resume \
    "$@"
